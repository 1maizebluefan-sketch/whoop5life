...
public final class BLEManager: NSObject, ObservableObject {
    ...
    private var model: WhoopDeviceModel = .unknown
    private var hasCustomProtocol = false
    ...
    func bootstrapStore() async {
        ...
        try? await store.upsertDevice(id: deviceId, mac: nil, name: state.deviceModel)
    ...
    public func connect() {
        ...
        log("Scanning for WHOOP custom/standard BLE services...")
        central.scanForPeripherals(
            withServices: [BLEManager.customService, BLEManager.heartRateService, BLEManager.batteryService],
...
extension BLEManager: CBCentralManagerDelegate {
    ...
    public func centralManager(_ central: CBCentralManager,
        ...
        let looksLikeWhoop = name.localizedCaseInsensitiveContains("WHOOP")
            || advertisedServices.contains(BLEManager.customService)
        guard looksLikeWhoop else { return }
        hasCustomProtocol = false
        state.customProtocolAvailable = false
        state.standardHeartRateAvailable = false
        state.standardBatteryAvailable = false
        model = WhoopDeviceModel.infer(from: name)
        state.deviceName = name
        state.deviceModel = model.rawValue
    ...
    public func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        ...
        for s in services {
            ...
            case BLEManager.customService:
                ...
                state.customProtocolAvailable = true
    ...
    public func peripheral(_ peripheral: CBPeripheral,
        ...
        for c in chars {
            ...
            case BLEManager.cmdWriteChar:
                guard model.supportsCustomProtocol || model == .unknown else {
                    log("Custom protocol present on \(state.deviceModel), but writes are disabled until verified")
                    continue
                }
                cmdCharacteristic = c
